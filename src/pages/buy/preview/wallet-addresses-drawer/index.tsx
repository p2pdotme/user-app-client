...
import type { WalletAddressBookError } from "@/core/client/wallet-address-book";
import { useWalletAddressBook } from "@/hooks";
import {
  type WalletAddressesPage,
  type WalletAddressFormData,
  walletAddressFormSchema,
} from "../shared";
...
export function WalletAddressesDrawer({
  children,
  onAddressSelected,
}: {
  ...
  const { addressBook, add, update, remove, setActive } =
    useWalletAddressBook();
  const [page, setPage] = useState<WalletAddressesPage>("list");
  ...
  const handleSave = async (values: WalletAddressFormData) => {
    try {
      if (page === "add") {
      ...
      const err = error as WalletAddressBookError;
