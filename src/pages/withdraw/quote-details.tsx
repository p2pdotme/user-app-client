...
import { WITHDRAW_PROGRESS_STEPS, type WithdrawState } from "./shared";
...
export function QuoteDetails({
  ...
  const handleDone = () => {
    ...
    if (withdrawState.status === "completed") {
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-muted-foreground text-xs">
                  {withdrawState.recipientAddress.slice(0, 10)}...
                  {withdrawState.recipientAddress.slice(-8)}
                </span>
              </div>
            </div>
  ...
  // Show failure state
  if (withdrawState.status === "failed") {
    return (
      <Card className="shadow-none">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
  ...
  // Show progress when withdraw is in progress
  if (["preparing", "approval", "swapping"].includes(withdrawState.status)) {
    const activeQuote = swapResponse?.route || rangoQuote?.route;
    ...
    return (
      <Card className="shadow-none">
        <CardHeader>
          <CardTitle>{t("WITHDRAW_PROGRESS")}</CardTitle>
        </CardHeader>
        <CardContent>
          <OrderProgress
            steps={WITHDRAW_PROGRESS_STEPS}
            currentStepKey={withdrawState.status}
            className="p-2"
          />
          ...
          {activeQuote?.path && (activeQuote.path?.length ?? 0) > 0 && (
            <div className="my-4">
              ...
              <div className="flex items-center justify-center gap-3 overflow-x-auto py-2">
                {/* Starting token (USDC on Base) */}
                <div className="flex min-w-0 flex-col items-center gap-1">
                  <div className="relative">
                    <ASSETS.ICONS.Usdc className="size-8" />
                    <ASSETS.ICONS.NetworkBase className="-right-1 -bottom-1 absolute size-3 rounded-full border border-background" />
                  </div>
                  <span className="text-center font-medium text-xs">USDC</span>
                </div>
  ...
  if (
    ...
    !withdrawState.destinationToken ||
    !withdrawState.destinationChain
  ) {
    return null;
  }
  ...
  // Show normal quote details
  const activeQuote = rangoQuote?.route;
  if (!activeQuote) {
    return (
      <Card className="shadow-none">
        <CardHeader>
        ...
        <CardContent>
          <Alert>
            <AlertCircle className="h-4 w-4" />
            <AlertDescription>{t("NO_ROUTE_FOUND")}</AlertDescription>
          </Alert>
        </CardContent>
      </Card>
    );
        ...
        <div className="space-y-2 text-sm">
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="text-muted-foreground">
              {activeQuote.estimatedTimeInSeconds
                ? moment
                    .duration(activeQuote.estimatedTimeInSeconds, "seconds")
                    .humanize()
                : "-"}
            </span>
          </div>
          ...
          {withdrawState.recipientAddress && (
            <div className="flex items-center justify-between">
              <span className="font-medium">{t("RECIPIENT")} </span>
              <span className="text-muted-foreground text-xs">
                {withdrawState.recipientAddress.slice(0, 10)}...
