...
import { useAnalytics, useCancelledTimestamp } from "@/hooks";
...
export function PayCancelled({ order }: { order: Order }) {
  ...
  // Track pay transaction cancelled view
  useEffect(() => {
  ...
  return (
    <>
      <main className="container-narrow flex h-full w-full flex-col gap-4 overflow-y-auto">
        ...
        <Card className="w-full gap-4 border-none bg-warning/20 shadow-none sm:gap-4">
          <CardContent className="space-y-2">
            <p className="font-medium text-sm">{t("DONT_MISS_THE_TIMING")}</p>
            <ul className="list-disc pl-4 font-light text-sm">
              <li>
                <p>{t("DYNAMIC_QR_EXPIRES_QUICKLY")}</p>
              </li>
              <li>
                <p>
                  {t(
