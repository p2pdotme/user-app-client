...
export function BuyPlaced({ order }: { order: Order }) {
    ...
    <main className="flex h-full w-full flex-col overflow-y-auto pt-4">
      <section className="container-narrow flex flex-col items-center justify-center gap-4 py-8">
        ...
        <h2 className="text-center font-medium text-lg">
          {t("CONNECTING_YOU_TO_A_MERCHANT_FOR_PAYMENT")}...
        </h2>
        ...
        <Card className="mx-4 gap-2 shadow-none sm:gap-4">
          ...
          <CardContent>
            <div className="space-y-2 text-sm">
              ...
              {(() => {
                // Use actualUsdcAmount directly (includes fees deducted)
                const receiveAmount = actualUsdcAmount;
          ...
          <CardFooter className="flex flex-col gap-2">
            ...
            <div className="flex w-full items-center justify-between">
              <span className="font-medium">{t("TOTAL_PAYABLE_AMOUNT")}</span>
              <span className="text-muted-foreground">
                {isLoadingOrderFeeDetails || orderFeeDetailsError ? (
                  <Skeleton className="h-5 w-20" />
                ) : actualFiatAmount ? (
